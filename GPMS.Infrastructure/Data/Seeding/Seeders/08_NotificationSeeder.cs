    ...
    public async Task SeedAsync()
    {
        try 
        {
            ...
            var evaluations = await _context.Evaluations
                ...
                .Where(e => e.ReviewRound != null && e.ReviewRound.SemesterID == activeSemester.SemesterID)
                .ToListAsync();
            ...
            {
                if (eval.Group == null || eval.Group.GroupMembers == null) continue;
                
                var leader = eval.Group.GroupMembers.FirstOrDefault(gm => gm.RoleInGroup == GroupRole.Leader);
                if (leader != null && !string.IsNullOrEmpty(leader.UserID))
                {
                    notifications.Add(new Notification
                    {
                        RecipientID = leader.UserID,
                        Title = "Đã có nhận xét mới",
                        Content = $"Phản hồi cho Review Round {(eval.ReviewRound != null ? eval.ReviewRound.RoundNumber : 1)} của nhóm bạn đã được cập nhật.",
        ...
        {
            Console.WriteLine($"[Seeding] NotificationSeeder FAILED: {ex.Message}");
        }
    }
}
